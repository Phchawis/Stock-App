    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, ...style }}>
      ...
      <div style={{ position: 'relative', display: 'flex' }}>
        <select id={selId} style={{
          ...
          background: rest.disabled ? 'var(--slate-100)' : 'var(--white)', border: '1px solid var(--border-default)',
          borderRadius: 'var(--radius-md)', font: 'var(--type-body)',
          color: 'var(--text-primary)', cursor: rest.disabled ? 'not-allowed' : 'pointer', outline: 'none',
        }}
          onFocus={(e) => { if (!rest.disabled) { e.currentTarget.style.borderColor = 'var(--border-brand)'; e.currentTarget.style.boxShadow = 'var(--shadow-focus)'; } }}
          onBlur={(e) => { if (!rest.disabled) { e.currentTarget.style.borderColor = 'var(--border-default)'; e.currentTarget.style.boxShadow = 'none'; } }}
          {...rest}
        >
