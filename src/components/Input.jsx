...
export function Input({
  label, hint, error, required = false, prefix = null, suffix = null,
  id, style, ...rest
}) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, ...style }}>
      ...
      >
        ...
        <input id={inputId} style={{
          ...
          font: 'var(--type-body)', color: 'var(--text-primary)', minWidth: 0,
        }} {...rest} />
        {suffix && <span style={{ color: 'var(--text-tertiary)', display: 'flex' }}>{suffix}</span>}
      </div>
      {(hint || error) && (
        <span style={{ font: 'var(--type-caption)', color: error ? 'var(--red-600)' : 'var(--text-tertiary)' }}>
          {error || hint}
        </span>
      )}
    </div>
